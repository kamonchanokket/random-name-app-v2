...
app = Flask(__name__)
app.secret_key = "nakhon_nayok_v11_chest_size"

DATA_FILE = "data.json"
ADMIN_PASSWORD = "qwertyuiop[]asdfghjkl" # รหัสเข้าหน้า Admin
...
<body class="min-h-screen pb-10 text-slate-200">
    <div class="max-w-md mx-auto p-4 pt-10">
        ...
        <div class="flex mb-6 bg-slate-900/80 p-1 rounded-2xl border border-white/5">
            ...
            <a href="/admin" class="flex-1 text-center py-3 rounded-xl text-sm font-bold {{ 'bg-orange-600 text-white' if page == 'admin' or page == 'login' else 'text-slate-500' }}">⚙️ แอดมิน</a>
        </div>
        ...
        <div class="glass-card rounded-[2.5rem] p-8 shadow-2xl border border-orange-500/20">
            ...
            {% if page == 'login' %}
                <form action="/admin_login" method="POST" class="text-center space-y-6">
                    ...
                    <input type="password" name="pw" placeholder="ADMIN PASSWORD" class="w-full bg-slate-950 border border-slate-800 rounded-xl py-4 text-center text-white outline-none focus:border-orange-500">
                    <button type="submit" class="w-full py-3 btn-primary rounded-xl font-bold text-white uppercase">Login</button>
                </form>

            {% elif page == 'admin' %}
                <div class="space-y-6">
                    <div>
                        <h3 class="text-orange-400 text-[10px] font-black mb-3 uppercase tracking-widest">เพิ่มสมาชิกและเลขอก</h3>
                        <form action="/add_member" method="POST" class="space-y-2 mb-4">
                            <input type="text" name="name" required placeholder="ชื่อเล่นเพื่อน" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white outline-none">
                            <div class="flex gap-2">
                                <input type="text" name="pin" required placeholder="PIN (4 หลัก)" class="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white outline-none">
                                <input type="text" name="size" required placeholder="เลขอก (เช่น 40-42)" class="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-white outline-none">
                            </div>
                            <button type="submit" class="w-full bg-orange-600 py-2 rounded-xl font-bold text-white mt-2">เพิ่มข้อมูล</button>
                        </form>
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto pr-1">
                            ...
                            <div class="bg-slate-800/40 border border-white/5 px-4 py-2 rounded-xl flex justify-between items-center text-[11px]">
                                ...
                                <a href="/del_member/{{ name }}" class="text-rose-500 font-bold text-lg">×</a>
                            </div>
                            {% endfor %}
                        </div>
                    </div>
                    ...
                    <div class="pt-6 border-t border-white/5">
                        <h3 class="text-emerald-400 text-[10px] font-black mb-3 uppercase tracking-widest">ประวัติการสุ่ม (แอดมินดูคนเดียว)</h3>
                        <div class="space-y-1">
                    ...
                    <div class="pt-4 border-t border-white/5 flex justify-between">
                        <a href="/logout" class="text-[9px] text-slate-500 underline uppercase">Logout</a>
                        <a href="/reset" class="text-[9px] text-rose-500 font-bold uppercase" onclick="return confirm('ล้างข้อมูลสุ่มใหม่หมด?')">Reset Draw</a>
                    </div>
                </div>
            ...
            {% else %}
                <div class="text-center space-y-6">
                    <div class="w-16 h-16 bg-orange-600/10 rounded-3xl flex items-center justify-center mx-auto border border-orange-500/20">
                        <i data-lucide="user-check" class="text-orange-500 w-8 h-8"></i>
                    </div>
                    
                    <form action="/draw" method="POST" class="space-y-4">
                        <div class="text-left">
                            <label class="text-[10px] font-black text-orange-400 uppercase tracking-widest ml-1">1. เลือกชื่อตัวเอง</label>
                        ...
                        <div class="text-left">
                            <label class="text-[10px] font-black text-orange-400 uppercase tracking-widest ml-1">2. ใส่รหัส PIN</label>
                        ...
                        <button type="submit" class="w-full py-4 mt-2 rounded-xl font-black text-lg btn-primary text-white shadow-xl uppercase tracking-widest">
                            สุ่มเลย!
                        </button>
                    </form>
                </div>
    ...
    {% if result %}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl">
        <div class="glass-card w-full max-w-xs p-10 rounded-[3rem] border-2 border-orange-500/30 text-center space-y-6">
            <p class="text-orange-500 text-[10px] font-black tracking-widest uppercase">มึงสุ่มได้...</p>
            ...
            <div class="py-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
                <p class="text-emerald-400 text-[10px] uppercase font-black mb-1">รายละเอียดหน้าอก</p>
                <p class="text-2xl font-black text-white italic">{{ result_size }}</p>
            </div>

            <button onclick="window.location='/'" class="bg-orange-600 w-full py-3 rounded-xl text-white font-bold text-xs uppercase">โอเค จำขึ้นใจ</button>
        </div>
    </div>
    {% endif %}
...
def index():
    data = load_data()
    available_names = [n for n in data["members"].keys() if n not in data["assignments"]]
    return render_template_string(HTML_TEMPLATE, available_names=sorted(available_names), page='index')

@app.route("/admin")
def admin():
    if not session.get("is_admin"): return render_template_string(HTML_TEMPLATE, page='login')
    data = load_data()
    return render_template_string(HTML_TEMPLATE, members=data["members"], assignments=data["assignments"], page='admin')
...
def draw():
    ...
    if user in data["assignments"]:
        ...
        return render_template_string(HTML_TEMPLATE, page='index', result=res_name, result_size=data["members"][res_name]["size"])
    ...
    if not candidates:
        return "<script>alert('ไม่มีคนให้สุ่มเหลือแล้ว!'); window.location='/';</script>"

    target = random.choice(candidates)
    data["assignments"][user] = target
    save_data(data)
    
    return render_template_string(HTML_TEMPLATE, page='index', result=target, result_size=data["members"][target]["size"])
