    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                # รองรับข้อมูลเก่าที่ยังไม่มีฟิลด์ sizes
...
<head>
    ...
    <style>
        :root { --brand-orange: #ff6b35; --brand-dark: #0f172a; }
...
<body class="p-4 md:p-8">
    <div class="max-w-md mx-auto relative">
        <header class="text-center mb-10 relative">
            ...
            <p class="text-slate-400 text-sm font-light italic">"สุ่มหาคนที่จะโดนแกง... ด้วยเสื้อที่เห็นแล้วต้องร้องขอชีวิต"</p>
        </header>
            ...
            {% elif page == 'admin' %}
                <div class="space-y-8">
                    <section>
                        ...
                        <form action="/add_name" method="POST" class="space-y-2">
                            <input type="text" name="new_name" placeholder="ชื่อเล่นเพื่อน..." class="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500">
                            <div class="flex gap-2">
                                <input type="text" name="shirt_size" placeholder="Size เสื้อ (เช่น L, 42...)" class="flex-1 bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-500">
                    ...
                    <section>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-3">รายชื่อและไซส์เสื้อ ({{ names|length }})</label>
                        <div class="flex flex-wrap gap-2">
                            {% for name in names %}
                            <div class="group bg-slate-800/40 border border-white/5 px-4 py-2 rounded-xl text-sm flex items-center gap-3 transition-all">
                                <span class="font-medium text-slate-200">{{ name }} <span class="text-orange-400 text-xs">({{ sizes.get(name, 'ไม่ระบุ') }})</span></span>
                                <a href="/del_name/{{ name }}" class="text-rose-500 opacity-50 group-hover:opacity-100 hover:scale-110 transition-all"><i data-lucide="trash-2" class="w-4 h-4"></i></a>
                            </div>
                            {% endfor %}
                        </div>
                    </section>
                    ...
                    <section class="p-5 bg-rose-500/5 rounded-3xl border border-rose-500/10">
                        ...
                        <form action="/add_exclusion" method="POST" class="grid grid-cols-1 gap-3 mb-4">
                            ...
                            <button type="submit" class="w-full bg-slate-800 text-white py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest">บันทึกคู่ห้าม</button>
                        </form>
                        {% for pair in exclusions %}
                        <div class="flex justify-between items-center bg-slate-900/50 p-3 rounded-xl border border-white/5 mb-2 text-xs">
                            <span class="text-slate-400">{{ pair[0] }} ❌ {{ pair[1] }}</span>
                            <a href="/del_exclusion/{{ loop.index0 }}" class="text-rose-500"><i data-lucide="x" class="w-4 h-4"></i></a>
                        </div>
                        {% endfor %}
                    </section>

                    <footer class="pt-6 border-t border-white/10 flex flex-col gap-4">
                        <a href="/logout" class="text-center text-[10px] text-slate-500 hover:text-slate-300 underline font-bold">LOGOUT ADMIN</a>
                        <a href="/reset" class="py-3 px-6 bg-rose-950/30 border border-rose-500/20 text-rose-500 rounded-xl text-[10px] font-bold text-center" onclick="return confirm('ล้างประวัติสุ่มใหม่หมด?')">RESET ALL ASSIGNMENTS</a>
                    </footer>
                </div>
            ...
            {% else %}
                <div class="text-center py-4">
                    ...
                    <form action="/draw" method="POST" class="space-y-8">
                        <div class="space-y-4">
                            <label class="block text-xs font-bold text-orange-400 uppercase tracking-[0.3em]">มึงคือใครในแก๊งนี้?</label>
                            <select name="user_name" class="w-full bg-slate-950 border border-slate-800 rounded-3xl p-6 text-xl font-bold text-white text-center outline-none focus:ring-2 ring-orange-500/50 transition-all">
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
        <div class="glass-card w-full max-w-sm p-10 rounded-[3rem] border-2 border-orange-500/50 text-center space-y-6 shadow-[0_0_80px_-15px_rgba(249,115,22,0.4)]">
            <div>
            ...
            <div>
                <h2 class="text-6xl font-black text-white py-2 drop-shadow-lg">{{ result }}</h2>
                <div class="inline-block mt-2 px-4 py-1 bg-white/10 rounded-full border border-white/10 text-orange-400 font-bold">
                    Size เสื้อ: {{ result_size }}
                </div>
            </div>
            
            <div class="py-4 px-6 bg-white/5 rounded-2xl border border-white/5 space-y-2">
                <p class="text-orange-400 text-xs font-bold uppercase tracking-tight">Mission Objective:</p>
                <p class="text-slate-300 text-xs leading-relaxed">ไปหาชุดที่มันเห็นแล้วต้องหลั่งน้ำตา! ส่วนข้อความข้างบนนี้เป็นแค่ไอเดียเสนอเฉยๆ มึงไม่ต้องทำตามก็ได้ เอาที่มึงมีไอเดียเลยจ้า กุเป็นแค่ AI กุไม่โกรธหรอก 🤖✨</p>
            </div>
            
            <button onclick="window.location='/'" class="w-full py-4 bg-slate-800 hover:bg-slate-700 rounded-2xl text-white text-xs font-bold uppercase tracking-widest transition-all">โอเค.. จะเหยียบไว้เป็นความลับ</button>
        </div>
    </div>
    {% endif %}
